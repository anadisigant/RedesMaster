...
class RedesMasterApp {
  constructor() {
    ...
    // Simulado Final State
    this.sfAnswers = new Array(40).fill(null);
    this.sfFlags = new Array(40).fill(false);
    this.sfActiveIdx = 0;
    this.sfTimerInterval = null;
    this.sfSeconds = 0;
  ...
  switchView(viewName) {
    ...
    switch (viewName) {
      ...
      case 'simulado':
        ...
        break;
      case 'simulado-final':
        mainTitle.innerText = "Simulado Final (ENADE / POSCOMP)";
        subTitle.innerText = "Responda a 40 questões simuladas no formato oficial com gabarito comentado.";
        if (this.sfTimerInterval === null) {
          document.getElementById('simulado-final-intro').style.display = 'block';
          document.getElementById('simulado-final-quiz-screen').style.display = 'none';
          document.getElementById('simulado-final-result-screen').style.display = 'none';
          document.getElementById('simulado-final-review-screen').style.display = 'none';
        }
        break;
      case 'game':
  ...
  setupEventListeners() {
    ...
    // Simulado Final events
    const startSfBtn = document.getElementById('start-simulado-final-btn');
    if (startSfBtn) {
      startSfBtn.addEventListener('click', () => this.startSimuladoFinal());
    }

    const prevSfBtn = document.getElementById('sf-quiz-prev-btn');
    if (prevSfBtn) {
      prevSfBtn.addEventListener('click', () => this.navigateSimuladoFinal(-1));
    }

    const nextSfBtn = document.getElementById('sf-quiz-next-btn');
    if (nextSfBtn) {
      nextSfBtn.addEventListener('click', () => this.navigateSimuladoFinal(1));
    }

    const flagSfBtn = document.getElementById('sf-quiz-flag-btn');
    if (flagSfBtn) {
      flagSfBtn.addEventListener('click', () => this.toggleSimuladoFinalFlag());
    }

    const restartSfBtn = document.getElementById('sf-restart-btn');
    if (restartSfBtn) {
      restartSfBtn.addEventListener('click', () => {
        document.getElementById('simulado-final-result-screen').style.display = 'none';
        document.getElementById('simulado-final-intro').style.display = 'block';
      });
    }

    const reviewSfBtn = document.getElementById('sf-review-btn');
    if (reviewSfBtn) {
      reviewSfBtn.addEventListener('click', () => this.renderSimuladoFinalReview());
    }

    const reviewBackSfBtn = document.getElementById('sf-review-back-btn');
    if (reviewBackSfBtn) {
      reviewBackSfBtn.addEventListener('click', () => {
        document.getElementById('simulado-final-review-screen').style.display = 'none';
...
• Total de Hosts: 2^(32 - ${cidr}) - 2 = 2^${32 - cidr} - 2 = <b>${usableCount} hosts válidos</b>.
  ...
  // --- SIMULADO FINAL METHODS ---

  startSimuladoFinal() {
    this.sfAnswers = new Array(SIMULADO_FINAL_DB.length).fill(null);
    this.sfFlags = new Array(SIMULADO_FINAL_DB.length).fill(false);
    this.sfActiveIdx = 0;
    this.sfSeconds = 0;

    if (this.sfTimerInterval) clearInterval(this.sfTimerInterval);

    // Update UI panels
    document.getElementById('simulado-final-intro').style.display = 'none';
    document.getElementById('simulado-final-result-screen').style.display = 'none';
    document.getElementById('simulado-final-review-screen').style.display = 'none';
    document.getElementById('simulado-final-quiz-screen').style.display = 'block';

    // Build the grid buttons (1 to 40)
    const grid = document.getElementById('sf-question-grid');
    if (grid) {
      grid.innerHTML = '';
      SIMULADO_FINAL_DB.forEach((q, idx) => {
        const btn = document.createElement('button');
        btn.className = 'option-btn';
        btn.style.padding = '0.5rem';
        btn.style.textAlign = 'center';
        btn.innerText = idx + 1;
        btn.id = `sf-grid-btn-${idx}`;
        btn.addEventListener('click', () => this.jumpToSimuladoFinalQuestion(idx));
        grid.appendChild(btn);
      });
    }

    // Start Timer
    const timerDisplay = document.getElementById('sf-timer-display');
    if (timerDisplay) timerDisplay.innerText = "00:00";
    this.sfTimerInterval = setInterval(() => {
      this.sfSeconds++;
      const mins = Math.floor(this.sfSeconds / 60).toString().padStart(2, '0');
      const secs = (this.sfSeconds % 60).toString().padStart(2, '0');
      if (timerDisplay) timerDisplay.innerText = `${mins}:${secs}`;
    }, 1000);

    this.renderSimuladoFinalQuestion();
  }

  renderSimuladoFinalQuestion() {
    const card = document.getElementById('sf-quiz-question-card');
    if (!card) return;

    const q = SIMULADO_FINAL_DB[this.sfActiveIdx];
    const prevAnswer = this.sfAnswers[this.sfActiveIdx];

    // Progress counter and bar
    document.getElementById('sf-quiz-progress-text').innerText = `Questão ${this.sfActiveIdx + 1} de ${SIMULADO_FINAL_DB.length}`;
    const progressPercent = ((this.sfActiveIdx + 1) / SIMULADO_FINAL_DB.length) * 100;
    document.getElementById('sf-quiz-progress-fill').style.width = `${progressPercent}%`;

    // Manage button visibilities and texts
    const prevBtn = document.getElementById('sf-quiz-prev-btn');
    if (prevBtn) {
      prevBtn.style.visibility = this.sfActiveIdx === 0 ? 'hidden' : 'visible';
    }

    const nextBtn = document.getElementById('sf-quiz-next-btn');
    if (nextBtn) {
      nextBtn.innerText = this.sfActiveIdx === SIMULADO_FINAL_DB.length - 1 ? 'Finalizar' : 'Avançar';
    }

    // Render Question and options
    let optionsHTML = '';
    q.options.forEach((opt, idx) => {
      let selectedClass = prevAnswer === idx ? 'correct' : '';
      optionsHTML += `
        <button class="option-btn ${selectedClass}" onclick="app.selectSimuladoFinalOption(${idx})">${opt}</button>
      `;
    });

    card.innerHTML = `
      <span class="question-badge" style="background: rgba(168,85,247,0.15); color: var(--color-intro)">ENADE / POSCOMP</span>
      <div class="question-text" style="line-height:1.6">${q.question}</div>
      <div class="options-list">
        ${optionsHTML}
      </div>
    `;
    ...
    const flagBtn = document.getElementById('sf-quiz-flag-btn');
    if (flagBtn) {
      if (this.sfFlags[this.sfActiveIdx]) {
    ...
    // Update Grid buttons styling
    SIMULADO_FINAL_DB.forEach((_, idx) => {
      const btn = document.getElementById(`sf-grid-btn-${idx}`);
      if (btn) {
        // Reset classes
        btn.className = 'option-btn';
        
        // Active highlight
        if (idx === this.sfActiveIdx) {
        ...
        // Answered check
        if (this.sfAnswers[idx] !== null) {
          btn.classList.add('correct');
        }

        // Flagged check
        if (this.sfFlags[idx]) {
          btn.style.border = '2px dashed var(--warning)';
        } else {
          btn.style.border = '';
        }
      }
  ...
  selectSimuladoFinalOption(optIdx) {
    this.sfAnswers[this.sfActiveIdx] = optIdx;
    
    // Highlight option locally
    const btns = document.querySelectorAll('#sf-quiz-question-card .option-btn');
    btns.forEach((btn, idx) => {
      if (idx === optIdx) {
        btn.classList.add('correct');
      } else {
    ...
    // Update grid button
    const gridBtn = document.getElementById(`sf-grid-btn-${this.sfActiveIdx}`);
    if (gridBtn) {
      gridBtn.classList.add('correct');
    }
  }

  toggleSimuladoFinalFlag() {
    this.sfFlags[this.sfActiveIdx] = !this.sfFlags[this.sfActiveIdx];
    this.renderSimuladoFinalQuestion();
  }

  jumpToSimuladoFinalQuestion(idx) {
    this.sfActiveIdx = idx;
    this.renderSimuladoFinalQuestion();
  }

  navigateSimuladoFinal(direction) {
    if (direction === 1) {
      if (this.sfActiveIdx === SIMULADO_FINAL_DB.length - 1) {
        // Check if there are unanswered questions
        const unansweredCount = this.sfAnswers.filter(ans => ans === null).length;
        if (unansweredCount > 0) {
          if (!confirm(`Você tem ${unansweredCount} questões sem resposta. Deseja finalizar o simulado mesmo assim?`)) {
            return;
          }
        }
        this.finishSimuladoFinal();
      } else {
        this.sfActiveIdx++;
        this.renderSimuladoFinalQuestion();
      }
    } else if (direction === -1) {
      if (this.sfActiveIdx > 0) {
        this.sfActiveIdx--;
        this.renderSimuladoFinalQuestion();
      }
    }
  ...
  finishSimuladoFinal() {
    ...
    let correct = 0;
    SIMULADO_FINAL_DB.forEach((q, idx) => {
      if (this.sfAnswers[idx] === q.correctAnswer) {
        correct++;
      }
    });

    const percent = Math.round((correct / SIMULADO_FINAL_DB.length) * 100);

    // Update UI elements
    document.getElementById('sf-score-percent').innerText = `${percent}%`;
    document.getElementById('sf-result-correct-count').innerText = correct;
    document.getElementById('sf-result-wrong-count').innerText = SIMULADO_FINAL_DB.length - correct;

    const mins = Math.floor(this.sfSeconds / 60).toString().padStart(2, '0');
    const secs = (this.sfSeconds % 60).toString().padStart(2, '0');
    document.getElementById('sf-score-details-text').innerText = `Você acertou ${correct} de ${SIMULADO_FINAL_DB.length} questões em ${mins}:${secs}.`;

    // Apply color class to circle
    const circle = document.getElementById('sf-score-circle-element');
    circle.className = 'score-circle';
    const feedbackText = document.getElementById('sf-score-feedback-text');

    if (percent >= 80) {
      circle.classList.add('success');
      feedbackText.innerText = "Excelente desempenho! Nível avançado consolidado.";
    } else if (percent >= 50) {
      circle.classList.add('warning');
      feedbackText.innerText = "Bom esforço! Revise as questões incorretas para fixar.";
    } else {
      circle.classList.add('error');
      feedbackText.innerText = "Atenção: Recomendamos rever o material teórico e refazer o simulado.";
    }
  ...
  renderSimuladoFinalReview() {
    const container = document.getElementById('sf-review-cards-container');
    if (!container) return;

    container.innerHTML = '';

    SIMULADO_FINAL_DB.forEach((q, idx) => {
      const userAns = this.sfAnswers[idx];
      const isCorrect = userAns === q.correctAnswer;

      const card = document.createElement('div');
      card.className = 'concept-block';
      card.style.borderLeft = isCorrect ? '5px solid var(--success)' : '5px solid var(--error)';
      ...
      q.options.forEach((opt, optIdx) => {
        ...
        if (optIdx === q.correctAnswer) {
          ...
          style = 'background: rgba(16, 185, 129, 0.12); border-color: var(--success); color: #a7f3d0; cursor: default;';
          badge = ' <span class="matching-badge" style="background:var(--success); color:#fff; font-size: 0.65rem; margin-left: auto;">GABARITO</span>';
        } else if (optIdx === userAns) {
          // Highlight user's incorrect selection in red
          style = 'background: rgba(239, 68, 68, 0.12); border-color: var(--error); color: #fca5a5; cursor: default;';
          badge = ' <span class="matching-badge" style="background:var(--error); color:#fff; font-size: 0.65rem; margin-left: auto;">SUA ESCOLHA</span>';
        } else {
          style = 'opacity: 0.7; cursor: default; pointer-events: none;';
        }

        optionsHTML += `
          <div class="option-btn" style="display: flex; align-items: center; ${style}">
            ${opt}
            ${badge}
          </div>
        `;
      ...
      const statusBadge = isCorrect 
        ? `<span class="module-badge" style="color: var(--success); background: rgba(16, 185, 129, 0.15)">Acertou</span>`
        : `<span class="module-badge" style="color: var(--error); background: rgba(239, 68, 68, 0.15)">Errou</span>`;

      card.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
          <span class="question-badge" style="margin: 0; background: rgba(255,255,255,0.05); color: #fff;">Questão ${idx + 1}</span>
          ${statusBadge}
        </div>
        <div class="question-text" style="font-size: 1rem; margin-bottom: 1.25rem; line-height: 1.6;">${q.question}</div>
        <div class="options-list" style="margin-bottom: 1.25rem;">
          ${optionsHTML}
        </div>
        <div class="explanation-block" style="display: block; border-left-color: var(--color-accent); margin-top: 1rem; background: rgba(99,102,241,0.05);">
          <strong>💡 Explicação Acadêmica:</strong><br>
          <span style="color: #e5e7eb; font-size: 0.9rem; line-height:1.6; display:inline-block; margin-top:0.5rem;">
    ...
    document.getElementById('simulado-final-result-screen').style.display = 'none';
    document.getElementById('simulado-final-review-screen').style.display = 'block';
  }
}
